---
name: author-skin
description: Turn an approved Snaketron prototype and manifest into a validated SkinDoc v2 implementation using layers, textures, sprite sheets, or a hybrid. Use for factory builds and human-authorized skin implementation; do not use for prototype approval or publication.
---

# Author a Snaketron skin

Build the exact human-approved prototype as a SkinDoc v2 revision. This package
is agent-neutral: use the inputs and output schemas supplied by the caller, not
an agent's transcript, private memory, or provider-specific executable.

## Required inputs

Stop with a typed `invalid_input` result if any item is absent or inconsistent:

- approved prototype bytes or a readable content-addressed reference;
- its prototype manifest and exact `prototype_approval` artifact hash;
- the repository-canonical design direction and literal human feedback;
- the generated capability manifest for the pinned schema/renderer SHA;
- output schemas, budgets, pure-tool allowlist, and artifact workspace.

The approval hash must equal the prototype image hash. A machine verdict or a
training label is not approval. Never substitute another candidate or silently
regenerate the approved direction.
...
5. If assets are needed, emit `generate_asset` tool requests whose `arguments`
   conform to
   ...
   when running as a factory task worker.
...
7. The driver journals every external operation, generates/repairs assets,
   ...
   neither worker nor driver overwrites their history. Do not upload, register,
   approve, publish, use Git, or mutate external storage yourself.
