...
/// What a skin document contains, as controls an editor can render.
///
/// The Builder walks this rather than hard-coding a form, and the descriptor is
/// generated by exhaustive destructuring in `skin-schema`, so a field added to
/// the document cannot go missing from the editor without breaking the build.
#[wasm_bindgen(js_name = skinDocSchema)]
pub fn skin_doc_schema() -> Result<String, JsValue> {
    serde_json::to_string(&skin_schema::describe::describe_skin_doc())
...
/// reference yet — it is not stored, so it has no name of its own. This
/// registers it under a scratch id instead, replacing whatever was there
/// before, so the preview shows the edit rather than the last saved version.
#[wasm_bindgen(js_name = registerDraftSkin)]
pub fn register_draft_skin(handle: &str, document_json: &str) -> Result<(), JsValue> {
    crate::skin::registry::register_draft_skin(handle, document_json)
