...
const bundleManifestRaw = process.env.SNAKETRON_FACTORY_RENDERER_BUNDLE_MANIFEST;
...
const bundleManifest = parsePinnedRendererBundle(bundleManifestRaw, bundleManifestSha);
...
page.on('response', (response) => {
  ...
  rendererResponsePromises.push(
    response
      ...
      .catch((error) => ({ error: `could not read renderer asset ${response.url()}: ${error}` })),
  );
});

await page.goto(`${webUrl.replace(/\/$/, '')}/qa/skins`, { waitUntil: 'networkidle' });
await page.waitForFunction(() => window.wasm?.registerAuthoredSkin !== undefined);
...
if (servedRenderer.errors.length > 0) {
  throw new Error(`renderer bundle attestation failed: ${servedRenderer.errors.join('; ')}`);
}
...
if (!response.ok()) {
  throw new Error(`private skin fetch failed: HTTP ${response.status()} ${await response.text()}`);
}
...
const setup = await page.evaluate(
  ({ ref, documentJson, periodMs }) => {
    ...
    const poseByName = Object.fromEntries(fixtures.poses.map((pose) => [pose.name, pose]));
    ...
    for (const pose of requestedPoses) add(pose, 'own', 0, false, false, `pose: ${pose}`);
    add('longer_than_head_gradient', 'enemy', 0, true, false, 'state: enemy boost');
    add('zigzag', 'own', 0, false, true, 'state: dead turning');
    for (const [index, fraction] of [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875].entries()) {
      add(
        'longer_than_head_gradient',
        'own',
        fraction * periodMs,
    ...
    return { poses: requestedPoses, roles: fixtures.roles, tiles: tiles.length };
  },
  { ref: contentRef, documentJson: document, periodMs },
);

await page.waitForFunction(() => window.wasm.skinAssetsPending() === false, null, {
  timeout: 20_000,
});
await page.evaluate(() => window.__factoryPaint(0));
await page.waitForTimeout(200);

const assetStatus = await page.evaluate(() => JSON.parse(window.wasm.skinAssetsStatus()));
if (assetStatus.failed > 0) {
  throw new Error(`${assetStatus.failed} requested image assets failed to decode`);
}
...
for (let frame = 0; frame < 60; frame += 1) {
  await page.evaluate((clock) => window.__factoryPaint(clock), (frame / 60) * periodMs);
