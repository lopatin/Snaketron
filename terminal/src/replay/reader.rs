...
impl ReplayReader {
    pub fn load_replay(path: &Path) -> Result<ReplayData> {
        ...
        let metadata: ReplayMetadata = serde_json::from_str(&metadata_line)
            .context("Failed to parse replay metadata")?;
        
        // Parse initial state (second line)
        let initial_state_line = lines.next()
        ...
        for (i, line_result) in lines.enumerate() {
            let line = line_result
                .with_context(|| format!("Failed to read event line {}", i + 3))?;
