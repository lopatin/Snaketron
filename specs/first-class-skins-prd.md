...
- **Base skins become a second equip slot.** A base skin is a skin of kind `base` — today's viewer-attributed `BaseTheme` promoted to its own equippable entity. Attribution rules from `specs/skins-prd.md` section 5.6 are unchanged: bases are viewer-attributed and never leave the client, so the base slot needs no wire change at all; snake-skin-embedded base themes remain as the fallback beneath an explicitly equipped base skin. *(**Superseded** by `specs/base-skins-prd.md`, which redefines a base skin as a pair of pictures plus a text colour, attributes it to the team that owns the endzone, and does publish it on the wire. The slot, its stored `base:` form, and the theme fallback beneath it are unchanged.)*
...
- Skinning arena gameplay objects (food, NOS canisters, grid) — the boundary rulings of `specs/skins-prd.md` section 7 stand unamended except where section 5.2 below explicitly extends them (base as a separate entity kind), and except for base attribution, which `specs/base-skins-prd.md` amends outright.
...
> **Superseded in part by `specs/base-skins-prd.md`.** A base skin is no longer
> the `BaseTheme` vocabulary in its own document. It is a home picture, an away
> picture and a text colour; it is attributed to the team whose endzone it
> paints rather than to the viewer; and it therefore *does* enter `GameState`,
> in a fingerprint-excluded `team_bases` map. The claim above that the base slot
> "never enters `GameState`, the wire, or any other player's screen" is no
> longer true of a base skin — it remains exactly true of the per-snake colour
> themes, which are what an account equipped before that change still holds.
> Renderer-owned geometry is unchanged, and the hue locks are preserved by the
> authored home/away pair rather than relaxed.
...
| `POST /api/skins/{skin_id}/publish-request` | creator | Body `{revision, contentRef}` opens review for those exact immutable bytes and enters the sparse review-queue index. `DELETE` with the same exact body cancels only that request, never a newer one. Evaluation-namespace skins are refused. |
