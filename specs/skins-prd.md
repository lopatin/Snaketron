...
- Attribute world dressing deterministically: the **viewer's** skin themes the bases (zone tints, goal walls, endzone text colors — geometry and text layout stay generic), and the **scorer's** skin themes the goal celebration everyone sees, from a palette plus a first-party effect id. *(Amended by `specs/base-skins-prd.md`: a separate **base skin** — a pair of pictures and a text colour — is attributed to the team that owns the endzone and does travel. The colour themes described here are unchanged and still never leave the viewer.)* Classic themes reproduce today's bytes exactly, which also *resolves* the long-standing hardcoded-hex leak in `scoreEffects.ts` instead of deferring it.
...
6. Make the team base (zone tints, goal walls, endzone text colors) and the goal celebration skinnable via declarative themes — viewer-attributed for bases, scorer-attributed for celebrations — with classic themes byte-identical to today, resolving the `scoreEffects.ts` hardcoded-hex leak in the process. *(Team-attributed picture bases were added later on top of this; see `specs/base-skins-prd.md`.)*
...
> **Amended.** `specs/base-skins-prd.md` overturns the owner-attribution
> rejection below for a new entity — a **base skin**, which is a home picture,
> an away picture and a text colour, and which is attributed to the team whose
> endzone it paints. The paragraph as written remains exactly true of the
> colour `BaseTheme` it describes. Of the two reasons given for the rejection,
> the first (no deterministic owner) is dissolved by a defined resolution
> order, and the second (the friend-cool/foe-warm read) is preserved by
> authoring a home and an away kit and measuring the shipped pixels, rather
> than by trusting a picture to declare its own hue.
...
| Team bases (zones, walls, endzone text) | Skinnable: viewer-attributed color theme; geometry and classification generic. **Amended** — a team-attributed picture is painted over the theme (`specs/base-skins-prd.md`) |
...
10. **Team bases — skinnable, viewer-attributed.** Zone tints, goal-wall colors, and endzone text colors resolve from the viewer's skin `BaseTheme` (section 5.6); zone geometry, rotation handling, wall thickness, text content, fonts, and the which-side-is-friendly classification (`render.rs:1638-1642`) stay generic. Classic's theme is byte-identical to today's six hexes (`#e6f4fa`/`#ffe6e6`, `#7aa8c1`/`#c18888`, `#c0d8e4`/`#e4c0c0`), so goldens are unaffected. Hue locks keep friendly cool and enemy warm (section 5.4). Owner-attribution is rejected (no deterministic owner in 2v2; section 5.6). **Amended by `specs/base-skins-prd.md`:** a base *skin* — a picture, not a theme — is attributed to the team that owns the endzone and travels to every client. Everything this ruling makes generic stays generic; the picture supplies pixels for the zone rectangle and nothing else.
11. **Perspective classification — generic, always.** `SnakeRole` is computed only by the renderer from the same seven inputs `snake_palette` consumes today; the derivation table in section 5.2 is normative, including its viewer-dependent FFA slot remap. Skins map fully resolved roles to paint, and the same friendly-cool/enemy-warm rule binds base and celebration themes. The hue-window validator must exist before any non-classic skin is selectable in team games; a red-looking teammate is a competitive-integrity bug, not a style choice. This ruling's *guarantee* is **unamended** and binds base skins, but its mechanism moves: a picture cannot declare a hue, so nothing about a base skin's art says which end is which. The side read is carried entirely by renderer-owned cues a skin cannot reach — the goal wall, the hue-locked snakes, the names on each endzone, and where a player spawns — which honours "computed only by the renderer" more literally than asking art to declare its own side did. Requiring the art to declare it *was* tried and measured, and it turned seventeen base skins into seventeen shades of blue and red (`specs/base-skins-prd.md` sections 2.2 and 2.3).
...
| Both duel players run the same skin package | No conflict by construction: base *themes* are viewer-attributed, celebrations scorer-attributed. Base *skins* are per team, and a duel team is one player, so there is still nothing to resolve. |
