---------------------------- MODULE BroadcastChannelGeneric ----------------------------
...
\* This is a generic version that doesn't define the message type
...
GarbageCollect ==
    LET minPos == CHOOSE pos \in 0..Len(channel) : 
        /\ \A r \in ReceiverSet: receiverPositions[r] >= pos
        /\ \A p \in 0..Len(channel): (\A r \in ReceiverSet: receiverPositions[r] >= p) => p <= pos
    IN /\ minPos > 0
       /\ channel' = SubSeq(channel, minPos + 1, Len(channel))
       /\ receiverPositions' = [r \in ReceiverSet |-> receiverPositions[r] - minPos]

\* Helper for type checking - users should define ChannelMessageType
ChannelTypeInvariant(MessageType) ==
    /\ channel \in Seq(MessageType)
    /\ Len(channel) <= ChannelCapacity
