...
async fn test_solo_game_play_again() -> Result<()> {
    timeout(Duration::from_secs(45), async {
        ...
        match first_game_result {
            Ok(result) => {
                println!("✓ First game completed successfully with result: {:?}", result);
            }
            Err(e) => {
        ...
        match second_game_result {
            Ok(result) => {
                println!("✓ Second game completed successfully with result: {:?}", result);
            ...
            Err(e) => {
                ...
                panic!("Play-again functionality is broken: Second solo game creation failed with: {}", e);
            }
        }
...
async fn play_single_solo_game(client: &mut TestClient, user_id: i32) -> Result<GameCompletionResult> {
    ...
    let game_id = match creation_msg {
        ...
        _ => return Err(anyhow::anyhow!("Expected SoloGameCreated message, got {:?}", creation_msg)),
    };
        ...
        WSMessage::GameEvent(event) => {
            ...
            match event.event {
                GameEvent::Snapshot { game_state } => {
                    ...
                    assert_eq!(game_state.players.len(), 1, "Solo game should have exactly 1 player");
                    
                    // Get snake ID for the player
                    let snake_id = game_state.players.get(&(user_id as u32))
                        .expect("Player should have a snake").snake_id;
                    
                    println!("Initial game state - Status: {:?}, Snake ID: {}", game_state.status, snake_id);
                    (game_id, snake_id)
                }
    ...
    while start_time.elapsed() < Duration::from_secs(15) && !game_completed {
        ...
        match msg {
            Ok(Ok(WSMessage::GameEvent(event))) => {
                ...
                match &event.event {
                    GameEvent::SnakeDied { snake_id: died_snake_id } => {
                        assert_eq!(*died_snake_id, snake_id, "The died snake should be our snake");
