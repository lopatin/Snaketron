...
async fn test_multiple_servers_independent_games() -> Result<()> {
    ...
    for server_idx in 0..1 {
        ...
        println!(
            "Server {}: Waiting for matchmaking and game discovery...",
            server_idx
        );
            ...
            match client.receive_message().await {
                Ok(WSMessage::GameEvent(event)) => {
                    if matches!(event.event, GameEvent::Snapshot { .. }) {
                        ...
                        if joined_game_id != Some(game_id) {
                            ...
                            println!(
                                "Received duplicate game snapshot for game {}, ignoring",
                                game_id
                            );
                        }
                    }
                ...
                Err(e) => {
                    ...
                    if !match_found {
                        ...
                        println!(
                            "Joined game {} but no events received yet...",
                            joined_game_id.unwrap()
                        );
                    } else {
