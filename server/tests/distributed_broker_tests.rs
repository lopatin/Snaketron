...
async fn test_distributed_broker_local_game() -> Result<()> {
        ...
        loop {
            if let Some(event) = client1.receive_game_event().await? {
                ...
                if let GameEvent::Snapshot { game_state } = event.event {
                    return Ok::<(u32, ::common::GameState), anyhow::Error>((
                        event.game_id,
                        game_state,
                    ));
                }
    ...
    println!(
        "Both clients matched and auto-joined game {} with type {:?}",
        game_id, game_state1.game_type
    );
    ...
    assert_eq!(
        game_state1.game_type, game_state2.game_type,
        "Both clients should have same game type"
    );
...
async fn test_game_lifecycle_with_cleanup() -> Result<()> {
    ...
    // Create a match
    println!(
        "test_game_lifecycle_with_cleanup: Queuing players with IDs {} and {}",
        user1_id, user2_id
    );
    // client1.send_message(WSMessage::QueueForMatch {
    ...
    let game_id = match timeout(Duration::from_secs(5), async {
        loop {
            if let Some(event) = client1.receive_game_event().await? {
                println!(
                    "test_game_lifecycle_with_cleanup: Received event: {:?}",
                    event.event
                );
