...
fn test_initial_food_spawning() -> Result<()> {
    ...
    let mut game_state = GameState::new(40, 40, GameType::FreeForAll { max_players: 4 }, Some(12345), 0);
    ...
    assert_eq!(game_state.arena.food.len(), 10, "Should spawn exactly 10 food items initially");
    
    println!("Test passed: Initial food spawning created {} food items", game_state.arena.food.len());
...
fn test_food_never_spawns_in_endzones() -> Result<()> {
    ...
    let (left_bound, right_bound) = game_state.arena.main_field_bounds()
    ...
    for food_pos in &game_state.arena.food {
        assert!(
            ...
            food_pos.x, left_bound, right_bound
        );
    }
    ...
    println!("Test passed: {} food items spawned, all within main field", game_state.arena.food.len());
