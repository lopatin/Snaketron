...
use crate::matchmaking_manager::{MatchNotification, MatchmakingManager};
...
pub async fn remove_from_matchmaking_queue(
    ...
    if let Some(game_type) = removed_type {
        info!(
            "User {} removed from matchmaking queue for {:?}",
            user_id, game_type
        );
    } else {
...
pub async fn subscribe_to_match_notifications(
    ...
    let client =
        Client::open(redis_url).context("Failed to create Redis client for notifications")?;
    ...
    tokio::spawn(async move {
        ...
        while let Some(msg) = pubsub_stream.next().await {
            match msg.get_payload::<String>() {
                Ok(payload) => match serde_json::from_str::<MatchNotification>(&payload) {
                    Ok(notification) => {
...
pub async fn send_queue_position_update(
    ...
    if let Some(position) = matchmaking_manager
        .get_queue_position(user_id, game_type, queue_mode)
        .await?
    {
    ...
    pub async fn process_notifications(mut self) {
        while let Some(notification) = self.notification_rx.recv().await {
            let ws_message = match notification {
                MatchNotification::MatchFound { game_id, .. } => WSMessage::MatchFound { game_id },
                MatchNotification::QueueJoined {
                    position,
                    estimated_wait_seconds,
                ...
                MatchNotification::QueueLeft => WSMessage::QueueLeft,
            };
